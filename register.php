...
if($_SERVER["REQUEST_METHOD"]=="POST"){
	...
	if(empty($_POST["password"])||empty($_POST["email"])||empty($_POST["username"])){
		...
		while($data = $a->fetch_assoc()){
			$ada = $_POST["username"] == $data["username"] || $_POST["email"] == $data["email"];
		if($ada){
...
<body>
	<form style="width: 30vw;border: solid 1px black; border-radius: 3px; margin-top: 10vw; margin-left: 5vw; padding: 1vw;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"])?>" method = "POST">
		<span style="font-size: 2vw; margin-left: -0.2vw; ">Register</span>
		...
		<div class="form-group" style="margin-top: 2vw;">
			<label for="username" style="margin-left: -0.2vw;">Email</label>
		...
		<p><?php echo $error;?></p>
		<input type="submit" value="Register" class="btn btn-primary" style="margin-bottom: 2vw;">
		<a class="btn btn-danger" style="color: white; margin-top: -2vw;" href="login.php">Login</a>
	</form>
</body>
</html>